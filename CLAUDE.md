...
`DEPLOYMENT.md` is the generic, publishable guide. Host specifics (SSH user, paths, deploy key, mail, cron) live in `HOSTING.md`, which is **private**: `scripts/github-export.sh` publishes a history-free snapshot of HEAD to GitHub, excludes `HOSTING.md`, strips every `# BEGIN site-specific … # END site-specific` block (the krotze.com redirect in `public/.htaccess`) and refuses to push if any pattern from `.github-export-deny` (also private) survives. Keep host details in those two places only. Short version of a deploy: build + commit + `git push origin main`, then pull on the host (the non-interactive recipe is in `HOSTING.md`), and `php artisan migrate --force` there when migrations changed. Publishing to GitHub is a separate, deliberate step: `scripts/github-export.sh --dry-run` first, then without the flag.
